...
def register_webhook_on_startup() -> None:
    """
    Render automatically sets RENDER_EXTERNAL_URL to this service's public URL.
    We use that to self-register the Telegram webhook on boot, so nothing has
    to be run manually from a terminal.
    """
    render_url = os.environ.get("RENDER_EXTERNAL_URL")
    if not render_url:
        log.info("RENDER_EXTERNAL_URL not set (not running on Render?) — skipping webhook registration")
        return
    try:
        resp = requests.post(
        ...
        log.info("Webhook registration: %s %s", resp.status_code, resp.text)
    except requests.RequestException:
        log.exception("Failed to auto-register webhook")
...
def webhook_status():
    """Visit this in your phone's browser to confirm the webhook is registered correctly."""
    try:
        resp = requests.get(f"{TELEGRAM_API}/getWebhookInfo", timeout=10)
        return jsonify(resp.json()), 200
    except requests.RequestException:
        return jsonify(status="error", detail="could not reach Telegram"), 500
