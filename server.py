...
V0 → V0.1 SCOPE CHANGE: this now keeps a short in-memory conversation
history per process (see _HISTORY below) so "what did I mean by 'that'"
works across messages in the same chat. It resets on every Render
restart/redeploy — acceptable for a single-user assistant, not a durable
memory store. If this service is ever run with more than one gunicorn
worker, each worker has its OWN history and answers will look amnesiac
depending on which worker handles a given message — keep worker count at
1 unless this is rebuilt on a shared store (e.g. a small state file
synced the same way as everything else, or Redis).
"""
import os
import traceback
...
# ---- conversation memory (in-process, single chat) -------------------
# Plain list of {"role": "user"|"model", "parts": [{"text": ...}]} dicts —
# the exact shape google-genai's `contents` param accepts. Only final
# text turns are stored, not intermediate tool-call parts: this is for
# pronoun/topic continuity across messages, not a full replay log.
_HISTORY = []
MAX_HISTORY_EXCHANGES = 10  # user+model pairs kept; oldest dropped first

# Cache of the last sync.import_modules() result — see run_assistant(),
# only refreshed when sync.sync() actually pulled new files. Same
# single-process caveat as _HISTORY: per-worker, wiped on restart.
_cached_mods = None


def _append_history(user_text, model_text):
    _HISTORY.append({"role": "user", "parts": [{"text": user_text}]})
    _HISTORY.append({"role": "model", "parts": [{"text": model_text}]})
    cap = MAX_HISTORY_EXCHANGES * 2
    if len(_HISTORY) > cap:
        del _HISTORY[:-cap]
...
- Every get_market_briefing/get_world_state result includes a "coherence" \
block (aligned: true/false, stale_sources: [...]) computed from timestamps \
embedded in the system's own data files — this is the authoritative \
staleness signal, not generated_at (that field is just when THIS reply was \
computed, not when the underlying scan ran). If coherence.aligned is false, \
say plainly which sources are stale and that you're describing last-known \
state, not a live read. Compare timestamps to the current time given below.
- When Valentine states an opinion, prediction, or read on the market, do \
NOT default to agreeing. Check it against what the tools actually show and \
respond with three things kept visibly separate: what the data shows, what \
he said, and your own reconciling read — naming agreement, conflict, or \
"the system hasn't measured that" as plainly applies. Scrutinize claims the \
same way you'd want a second opinion to.
- You have access to recent turns of this conversation. Use them for \
continuity (pronouns, "that", follow-up questions) — but re-check tool data \
fresh each time rather than assuming an earlier answer in this conversation \
is still current; the underlying state may have changed since.
- If asked for a "situation report" / "sitrep" / "full report", structure \
the answer under whichever of these have something to say — skip empty \
ones rather than padding: Market (what's happening), Thesis (what the \
system believes), Intent (what it's doing about it), Uncertainty (what's \
unresolved), Activity (what changed recently), System (anything unhealthy), \
Attention (what deserves a look).
"""
...
def build_tools(mods):
    ...
    def get_market_briefing() -> dict:
        ...
        # build_market_briefing()'s own "generated_at" is when THIS call ran,
        # not when the underlying scan ran — the real staleness signal is
        # WorldState's own coherence block, computed from timestamps embedded
        # in the actual data files. Pass it through since briefing drops it.
        briefing["coherence"] = ws.get("coherence")
        briefing["sources"] = ws.get("sources")
        return briefing
    ...
    def get_system_snapshot() -> dict:
        """Coarse system health: last known stats.json contents, source
        coherence/staleness, and how many analysis log entries exist. Use
        for 'is everything running ok' / 'is the bot alive' questions, not
        market questions.
        """
...
def run_assistant(user_text):
    global _cached_mods
    changed = sync.sync()
    # Only re-import (which is expensive — min_scanner.py alone is huge)
    # when sync() actually pulled fresh files. Before this fix, every
    # single message forced a full importlib.reload() of all 5 modules
    # even when nothing had changed, which is very likely what triggered
    # Render's free-tier (512MB) worker OOM-kill.
    if changed or _cached_mods is None:
        _cached_mods = sync.import_modules()
    mods = _cached_mods
    tools = build_tools(mods)

    # The model has no built-in sense of "now" — without this, it can't
    # judge whether a coherence/freshness timestamp it reads back is fresh.
    now_utc = datetime.now(timezone.utc).isoformat()
    system_instruction = SYSTEM_PROMPT + f"\n\nCurrent time: {now_utc} (UTC)."

    contents = _HISTORY + [{"role": "user", "parts": [{"text": user_text}]}]
    ...
    if text:
        _append_history(user_text, text)
        return text
    ...
    # Deliberately NOT added to history — an empty/failed turn isn't
    # something later messages should treat as "what I said last".
    finish_reason = None
...
def telegram_webhook():
    ...
    if text.strip().lower() in ("/reset", "/forget", "/new"):
        _HISTORY.clear()
        send_telegram("Conversation memory cleared — starting fresh.")
        return jsonify({"ok": True})
