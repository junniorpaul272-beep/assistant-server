...
- Every tool result includes state_last_updated_utc — when state.json was \
actually last committed, which is NOT necessarily recent. Before describing \
anything as the current market situation, compare that timestamp to the \
current time given below and say plainly how stale the data is (e.g. "this \
is from Friday, the scanner hasn't run since — treat it as last-known-state, \
not live"). Never present old data as if it just happened.
"""
...
def build_tools(mods):
    ...
    def get_market_briefing() -> dict:
        ...
        briefing["state_last_updated_utc"] = sync.get_state_last_updated()
        return briefing
    ...
    def get_world_state() -> dict:
        ...
        state = sync.load_json("state.json", default={})
        ws = mods["min_scanner"].build_world_state(state)
        ws["state_last_updated_utc"] = sync.get_state_last_updated()
        return ws
    ...
    def get_system_snapshot() -> dict:
        ...
        return {
            ...
            "state_last_updated_utc": sync.get_state_last_updated(),
        }
