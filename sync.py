...
# Timestamp of the last commit that touched state.json, refreshed each
# sync() — see get_state_last_updated(). None until the first successful
# sync, or if the lookup fails (rate-limited, network hiccup).
_state_last_updated = None
...
def _get_state_commit_time(sha):
    """When was state.json actually last written, per its own commit
    history — not when we happened to fetch it. This is what lets the
    Assistant tell 'the scanner ran an hour ago' from 'the scanner hasn't
    run since Friday' instead of narrating whatever's on disk as current.
    Raises on failure — callers must catch."""
    headers = {"Accept": "application/vnd.github+json"}
    if GITHUB_TOKEN:
        headers["Authorization"] = f"Bearer {GITHUB_TOKEN}"
    params = {"path": "state.json", "sha": sha or BRANCH, "per_page": 1}
    resp = requests.get(f"{API_BASE}/commits", headers=headers, params=params, timeout=10)
    resp.raise_for_status()
    commits = resp.json()
    if not commits:
        return None
    return commits[0]["commit"]["committer"]["date"]  # ISO 8601 UTC, e.g. "2026-08-28T14:03:11Z"


def get_state_last_updated():
    """UTC ISO timestamp of the last commit to state.json, or None if
    unknown. Tool functions should surface this in every result so the
    LLM can judge staleness instead of assuming the data is current."""
    return _state_last_updated
...
def sync(force=False):
    ...
    """
    global _last_sync, _state_last_updated
    now = time.time()
    ...
    try:
        _state_last_updated = _get_state_commit_time(sha)
    except requests.RequestException as e:
        print(f"sync: could not resolve state.json's last commit time ({e}); "
              f"staleness checks will be unavailable this cycle.")
        _state_last_updated = None
